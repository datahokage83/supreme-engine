...
//       try {
//         // Fetch site logo
//         const homeRes = await fetch('http://localhost:1337/api/home-page?populate=*');
...
//           `http://localhost:1337/api/segments?filters[SegmentSlug][$eq]=${encodeURIComponent(
//             slug
//           )}&populate=*`
//         );
...
//             ?.split(/\s{2,}|\n+/) // split by 2+ spaces or newlines
//             .filter((para: string) => para.trim().length > 0)
//             .map((para: string, index: number) => (
//               <p key={index} className="text-3xl tracking-wide font-normal leading-relaxed mb-8">
//                 {para.trim()}
//               </p>
//             ))}
//       </section>
...
//       <section>
//          <div className='bg-gray-200 min-h-screen border-l border-gray-400 py-12 px-10'>
//            <p>Right Section</p>
//          </div>
//       </section>
//       </div>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // -------------------------------------
        const imgRes = await fetch("http://localhost:1337/api/segment-image?populate=*");
